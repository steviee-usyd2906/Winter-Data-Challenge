...
35C (`hot_days_ge35`) and 40C (`hot_days_ge40`), then averages those
...
| **Heat anomaly (post-fire)** | `mean(hot days >=35C from Y to Y+3) - that division's available-data mean annual hot-day count` -- measured relative to each division's own baseline, so it captures "unusually hot for this place" rather than just "a hot place." |
